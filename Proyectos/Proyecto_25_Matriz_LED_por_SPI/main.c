/* =======================================================================
 * Proyecto 25:
 * Programa que muestra el algoritmo para demostrar la operación del
 * controlador para matriz de LED 8x8 de referencia MAX7219 que opera por
 * comunicación SPI. El algoritmo desarrollado realiza tres tareas
 * diferentes controladas por tres pulsadores N.A. que muestran las
 * siguientes funcionalidades del controlador:
 *      Presenta una tabla de 256 caracteres que tengan su equivalente con
 *      la tabla de caracteres ASCII estándar.
 *      Presenta un mensaje almacenado en la memoria mostrado letra a
 *      letra, usando la tabla de caracteres anterior.
 *      Presenta el mensaje anterior desplazándolo de derecha izquierda en
 *      la matriz de LEDs de manera repetitiva.
* ========================================================================
*/
...
void Ini_Matriz(void){ //Comandos de configuración del controlador
uint16 i;    
    SPIM_WriteTxData(escaneo+0x07); //Habilita las 8 columnas de la matriz
    CyDelayUs(50);
    SPIM_WriteTxData(modo+0x00); //Registro de modo de decodificación
    CyDelayUs(50);               //se selecciona no decodificar (es matriz)
    SPIM_WriteTxData(test+0x00); //No realizar test a la matriz
    CyDelayUs(50);
    SPIM_WriteTxData(shutdown+0x01); //Modo de operación normal (no shutdown)
    CyDelayUs(50);
    SPIM_WriteTxData(intensidad+0x04); //Brillo de la matriz medio-bajo (00-0F)
    CyDelayUs(50);    
    for (i=0x0100;i<0x0900;i=i+0x0100){
        SPIM_WriteTxData(i); //Limpiar todas las columnas de la matriz
        CyDelayUs(50);
    }   
}

//Tabla con los 256 caracteres que pueden ser visualizados, organizada como
//un arreglo matricial de 256x8. Las filas representan el carácter y las
//columnas del arreglo matricial representan los 8 valores de cada una de
//las columnas de matriz de LEDs
const uint16 caracteres [256] [8] = {
...
uint8 tabla[]={"Proyecto No.25 con PSoC5LP"}; //Mensaje a visualizar
//El tamaño de la tabla depende de la cantidad de caracteres del
//mensaje a visualizar + 16 posiciones para iniciar y terminar con
//la matriz en blanco
uint8 mensaje[(sizeof(tabla)*8)+16]={0,0,0,0,0,0,0,0};  
                                        
CY_ISR(INT_Pulsadores){
    switch (Pulsadores_Read()){ //Lee estado de pulsadores 
        case 0b110:
            vis=1; //Estado que indica visualizar tabla
        break;
        case 0b101:
            vis=2; //Estado que indica visualizar mensaje por letras
        break;
        case 0b011:
            vis=3; //Estado que indica visualizar mensaje con desplazamiento
        break;
        default:
        break;
    }
...
int main(void)
{
    ...
    vis=0;
    pos=8;
    for (j=0;j<sizeof(tabla);j++){ //Convierte el mensaje a desplazar en un
        for(i=0;i<8;i++){   //arreglo unidimensional para usar en la matriz 
            mensaje[pos]=caracteres[tabla[j]][i];
            pos++;
        }
    }
    ...
    {
        switch(vis){
            case 1: //Visualiza toda la tabla de caracteres
            for(j=0;j<255;j++){ //Barre las 256 filas de la tabla de caracteres
                for(i=0;i<8;i++){ //Lee los 8 datos de cada fila
                    dato=(((i+1)*0x0100)+caracteres[j][i]);
                    SPIM_WriteTxData(dato); //Envía dato a la matriz
                    CyDelayUs(50); //Espera a que se ejecute comando
                }
                CyDelay(1000);
            }
            break;
            case 2: //Visualiza mensaje letra a letra
            for (j=0;j<sizeof(tabla);j++){ //Lee mensaje a visualizar
                for(i=0;i<8;i++){ //Lee los 8 datos de cada fila
                    dato=(((i+1)*0x0100)+caracteres[tabla[j]][i]);
                    SPIM_WriteTxData(dato);
                    CyDelayUs(50);
                }
                CyDelay(400); //Tiempo de espera entre cada letra
            }
            CyDelay(800); //Tiempo de espera entre cada inicio de mensaje
            break;
            case 3: //Visualiza mensaje con desplazamiento
            for (j=0;j<pos;j++){ //Lee tabla con mensaje a desplazar
                for(i=0;i<8;i++){ //Lee los 8 datos de cada fila
                    dato=(((i+1)*0x100)+ mensaje[j+i]);
                    SPIM_WriteTxData(dato);
                    CyDelayUs(50);
                }
                CyDelay(200); //Tiempo de desplazamiento del mensaje
            }
            break;
            default:
            break;
